...
from .exceptions import (
    NenDBError, NenDBConnectionError, NenDBTimeoutError,
    NenDBValidationError, NenDBAlgorithmError, NenDBResponseError
)
from .types import (
    GraphNode, GraphEdge, AlgorithmResult, BFSResult, 
    DijkstraResult, PageRankResult, AlgorithmStatus
)
...
class NenDBClient:
        ...
        if not skip_validation:
            try:
                ...
                raise NenDBConnectionError(
                    f"Failed to connect to NenDB server at {self.base_url}: {e}"
                )
    ...
    def _make_request(
        ...
        try:
            ...
            raise NenDBTimeoutError(f"Request to {endpoint} timed out after {self.timeout} seconds")
            ...
            raise NenDBResponseError(f"HTTP error on {endpoint}: {error_msg}", details=details)
        except Exception as e:
    ...
    def graph_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Graph statistics including node count, edge count, and available algorithms
